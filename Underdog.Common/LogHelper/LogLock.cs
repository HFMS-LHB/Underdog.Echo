...
namespace Underdog.Common.LogHelper
{
    public class LogLock
    {
        ...
        private static List<RequestInfo> GetRequestInfo(ReadType readType)
        {
            ...
            var accessLogs = ReadLog(Path.Combine(_contentRoot, "Logs"), "RequestIpInfoLog_", Encoding.UTF8, readType).ObjToString();
            try
            {
        ...
        public static List<LogInfo> GetLogData()
        {
            ...
            try
            {
                var aoplogContent = ReadLog(Path.Combine(_contentRoot, "Logs"), "AOPLog_", Encoding.UTF8, ReadType.Prefix);
            ...
            try
            {
                var exclogContent = ReadLog(Path.Combine(_contentRoot, "Logs"), $"GlobalExceptionLogs_{DateTime.Now.ToString("yyyMMdd")}.log", Encoding.UTF8);
            ...
            try
            {
                var sqllogContent = ReadLog(Path.Combine(_contentRoot, "Logs"), "SqlLog_", Encoding.UTF8, ReadType.PrefixLatest);
