using Underdog.Common.Const;
using Underdog.Common.DB;
    ...
    public class DBSeed
    {
        ...
        public static async Task SeedAsync(MyContext myContext, string contentRootPath)
        {
            try
            {
                ...
                var referencedAssemblies = System.IO.Directory.GetFiles(path, "Underdog.Model.dll")
                    .Select(Assembly.LoadFrom).ToArray();
                var modelTypes = referencedAssemblies
                    .SelectMany(a => a.DefinedTypes)
                    .Select(type => type.AsType())
                    .Where(x => x.IsClass && x.Namespace is "Underdog.Model.Models")
                    .ToList();
        ...
        public static void MigrationLogs(MyContext myContext)
        {
            ...
            var modelTypes = referencedAssemblies
                .SelectMany(a => a.DefinedTypes)
                .Select(type => type.AsType())
                .Where(x => x.IsClass && x.Namespace != null && x.Namespace.StartsWith("Underdog.Model.Logs"))
                .ToList();
